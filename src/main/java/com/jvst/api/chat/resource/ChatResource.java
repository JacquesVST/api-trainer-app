...
public class ChatResource {
	...
	public List<Chat> listarChatPorIdInstrutor(@PathVariable @ApiParam(value = Doc.PARAM_ID_PREFIX
	...
	public List<Chat> listarChatPorIdAluno(@PathVariable @ApiParam(value = Doc.PARAM_ID_PREFIX + "de aluno do chat"
	...
	public void cadastrarChat(@RequestBody @Valid @ApiParam(value = Doc.PARAM_ENTIDADE_PREFIX + "chat"
			+ Doc.PARAM_ENTIDADE_SUFFIX1) ChatForm chatForm) {
		this.chatService.cadastrarChat(chatForm);
	}

	@ApiOperation(value = "Deixa o chat inacessível pelo aluno")
	@DeleteMapping("/aluno/{idChat}")
	public void exclusaoAluno(@PathVariable @ApiParam(value = Doc.PARAM_ID_PREFIX + "do chat"
			+ Doc.PARAM_ID_SUFFIX3, example = "1") Long idChat) {
		this.chatService.excluirParaAluno(idChat);
	}

	@ApiOperation(value = "Deixa o chat inacessível pelo instrutor")
	@DeleteMapping("/instrutor/{idChat}")
	public void exclusaoInstrutor(@PathVariable @ApiParam(value = Doc.PARAM_ID_PREFIX + "do chat"
			+ Doc.PARAM_ID_SUFFIX3, example = "1") Long idChat) {
		this.chatService.excluirParaInstrutor(idChat);
	}
}
