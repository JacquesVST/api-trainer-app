...
public class SequenciaResource {
	...
	@ApiOperation(value = Doc.CADASTRAR_PREFIX + "uma nova sequência" + Doc.CADASTRAR_SUFFIX)
	@PostMapping
	public void cadastrarSequencia(@RequestBody @Valid @ApiParam(value = Doc.PARAM_ENTIDADE_PREFIX + "sequência"
			+ Doc.PARAM_ENTIDADE_SUFFIX1) Sequencia sequencia) {
		this.sequenciaService.salvarSequencia(sequencia);
	}
}
